...
namespace AgentFrameworkQuickStart
{
    ...
    public class CarMasterMemory : AIContextProvider
    {
        ...
        private readonly ProviderSessionState<CarPreference> _sessionState;

        public CarMasterMemory(IChatClient client, Func<AgentSession?, CarPreference>? stateInitializer = null)
            : base(null, null)
        {
            _innerClient = client;
            _sessionState = new ProviderSessionState<CarPreference>(
                stateInitializer ?? (_ => new CarPreference()),
                this.GetType().Name);
        }
        ...
        {
            var preference = _sessionState.GetOrInitializeState(context.Session);
            ...
            if (context.RequestMessages.Any(m => m.Role == ChatRole.User))
            {
                try
                {
                    ...
                    返回 JSON 格式如下：
                    {
                      ...
                      "MustHaves": ["需求点1", "需求点2"] (如果没有提到任何具体配置或需求，请返回空数组 [])
                    }
                    ...
                    if (extraction.Result != null)
                    {
                        ...
                        if (newInfo.BudgetMax > 0) preference.BudgetMax = newInfo.BudgetMax;

                        // 能源类型更新
                        if (!string.IsNullOrEmpty(newInfo.EnergyType) && newInfo.EnergyType != "未指定" && newInfo.EnergyType != "null")
                        {
                            preference.EnergyType = newInfo.EnergyType;
                        }
                        ...
                        if (newInfo.MustHaves != null && newInfo.MustHaves.Any())
                        {
                            ...
                            // 使用 HashSet 进行去重合并
                            var updatedList = preference.MustHaves.Union(validNewItems, StringComparer.OrdinalIgnoreCase).ToList();
                            preference.MustHaves = updatedList;
                        }

                        _sessionState.SaveState(context.Session, preference);
                    }
                }
                catch (Exception)
                {
    ...
    public class CarMasterAgent : BaseAgent
    {
        ...
        public async Task RunMasterAsync()
        {
            ...
            var chatClient = client.GetChatClient(modelProvider.ModelId).AsIChatClient();

            var chatHistoryProvider = new VectorChatHistoryProvider(_vectorStore);
            var contextProvider = new CarMasterMemory(chatClient);
            ...
            while (true)
            {
                ...
                // 实时展示结构化记忆（画像）
                var mem = contextProvider.GetPreference(session);
    ...
    // 辅助方法：从 CarMasterMemory 获取偏好
    public static class CarMasterMemoryExtensions
    {
        public static CarPreference? GetPreference(this CarMasterMemory provider, AgentSession session)
        {
            return null; // 由 ProviderSessionState 管理，需要通过 session 获取
        }
    }
}